...
use crate::types::{
    PgBouncerUser, PgBouncerUserSpec, PostgresRole, PostgresRoleStatus, StatusEncodedPassword,
};
...
pub async fn reconcile_postgres_role(
    resource: Arc<PostgresRole>,
    context: Arc<ContextData>,
) -> anyhow::Result<Action, Error> {
    run_reconciler(resource, context)
        .await
        .map_err(|e| e.into())
}
...
async fn run_reconciler(
    ...
    if resource.metadata.deletion_timestamp.is_some() {
        ...
        let pg_connection =
            get_postgres_connection(resource.as_ref(), context.kubernetes_client.clone()).await?;
            ...
            .is_none()
        {
            ...
            match pg_connection
                .execute(
                    &format!(
                        ...
                        pg_connection.database, resource.spec.role
                    ),
                    &[],
                )
                .await
            {
                ...
                Err(e) => {
                    warn!(
                        "Failed to revoke privileges on database {} from {}: {}",
                        pg_connection.database, resource.spec.role, e
                    );
    ...
    let mut resource =
        ensure_finalizer(resource.as_ref().clone(), context.kubernetes_client.clone()).await?;

    let password_text = if let Some(encoded_password) = resource
        .status
        .as_ref()
        .and_then(|s| s.encoded_password.as_ref())
    {
        if encoded_password.original == resource.spec.password {
            ...
            resource
                .spec
                .password
                .get_password_text(&resource.spec.role)
        }
    } else {
        resource
            .spec
    ...
    let postgres_role_api: Api<PostgresRole> =
        Api::namespaced(context.kubernetes_client.clone(), &namespace);
    let mut resource = postgres_role_api
        .patch_status(&name, &serverside, &Patch::Apply(resource))
        .await?;

    let pg_connection =
        get_postgres_connection(&resource, context.kubernetes_client.clone()).await?;

    let username = resource.spec.role.clone();
        ...
        .is_some()
    {
        ...
        pg_connection
            .execute(
                &format!("ALTER USER {username} WITH PASSWORD '{password_text}'"),
                &[],
            )
            .await?;
    } else {
        ...
        pg_connection
            .execute(
                &format!("CREATE USER {username} WITH PASSWORD '{password_text}'"),
                &[],
            )
            .await?;
    }
    ...
    if resource.spec.grant_role_to_admin_user == Some(true) {
        ...
        pg_connection
            .execute(
                &format!("GRANT {} TO {}", username, pg_connection.admin_username),
                &[],
            )
            .await?;
    }
    ...
    if let Some(extra_roles) = &resource.spec.extra_roles {
        for extra_role in extra_roles {
            ...
            pg_connection
                .execute(&format!("GRANT {} TO {}", extra_role, username), &[])
                .await?;
        }
    }

    info!(
        "Granting connect to {} to database {}",
        username, pg_connection.database
    );
    pg_connection
        .execute(
            &format!(
                "GRANT CONNECT ON DATABASE {} TO {}",
                pg_connection.database, username
            ),
            &[],
        )
        .await?;
    ...
    info!(
        "Connect granted to database {} for user {}",
        pg_connection.database, username
    );

    let desired_configuration_parameters = resource.spec.configuration_parameters.clone();
    if let Some(config_params) = &desired_configuration_parameters {
        for (param, value) in config_params {
            if resource.status.as_ref().is_some_and(|s| {
                s.configuration_parameters
                    .as_ref()
                    .is_some_and(|p| p.get(param) == Some(value))
            }) {
                info!(
                    "Configuration parameter {} for role {} is already set to {}, skipping",
            ...
            info!(
                "Setting configuration parameter {} to {} for role {}",
    ...
    let status = resource.status_mut();
    if let Some(status) = status {
        if let Some(existing_params) = &status.configuration_parameters {
            if let Some(desired_configuration_parameters) = &desired_configuration_parameters {
                for param in existing_params.keys() {
                    if !desired_configuration_parameters.contains_key(param) {
                        info!(
                            "Resetting configuration parameter {} for role {}",
                            param, username
                        );
                        pg_connection
                            .execute(&format!("ALTER ROLE {} RESET {}", username, param), &[])
                            .await?;
                    }
                }
            } else {
        ...
        status.configuration_parameters = desired_configuration_parameters;
    } else {
        *status = Some(PostgresRoleStatus {
            encoded_password: None,
            configuration_parameters: desired_configuration_parameters,
        });
    }

    resource.metadata.managed_fields = None;
    let resource = postgres_role_api
        .patch_status(&name, &serverside, &Patch::Apply(resource))
        .await?;
    ...
    if let Some(pg_bouncer_reference) = &resource.spec.register_in_pg_bouncer {
        info!(
            "Registering role {username} in pg_bouncer {}",
            pg_bouncer_reference.name
        );

        let pg_bouncer_users_api: Api<PgBouncerUser> =
            Api::namespaced(context.kubernetes_client.clone(), &namespace);
        ...
        pg_bouncer_users_api
            .patch(&name, &serverside, &Patch::Apply(pg_bouncer_user))
            .await?;
        info!(
            "Registered role {username} in pg_bouncer {}",
            pg_bouncer_reference.name
        );
    }
